/* ============================================================
   ...
   直したときの手順
     ページ（HTML）を直したら DOC_VERSION を1つ上げる。
     画像や書体を差し替えたときだけ ASSET_VERSION を上げる。
     版を上げた側の控えは全部捨てられるので、HTMLの手直しのたびに
     ASSET_VERSION まで上げると、せっかく貯めた写真の控え（最大400枚）を
     毎回捨てることになる。だから2つに分けてある。
     （HTMLは通信優先なので、上げ忘れても古いページは出ない）
   ============================================================ */

var DOC_VERSION   = 'nekoclub-doc-v34';    // ページを直したら上げる
var ASSET_VERSION = 'nekoclub-asset-v16';  // 写真・書体を差し替えたときだけ上げる
var CACHE_DOC = DOC_VERSION + '-page';     // ページ用
var CACHE_IMG = ASSET_VERSION + '-img';    // 写真用
var CACHE_FNT = ASSET_VERSION + '-font';   // 書体用
...
var DOCS = ['', 'index.html', 'album.html', 'photos.json'];   // photos.json はアルバムの写真一覧
  ...
  e.waitUntil(
    caches.keys().then(function (names) {
      return Promise.all(names.map(function (n) {
        if (n.indexOf(DOC_VERSION) !== 0 && n.indexOf(ASSET_VERSION) !== 0) return caches.delete(n);
      }));
...
function networkFirst(req) {
  return fetch(req).then(function (res) {
    ...
    return caches.match(req).then(function (hit) {
      if (hit) return hit;
      // 写真一覧の控えが無いときは、ページの控えを返さない（JSONとして読めないので）
      if (/\.json$/.test(new URL(req.url).pathname)) return new Response('{}', { status: 503, headers: { 'Content-Type': 'application/json' } });
      // caches.match は Promise を返すので、|| でつなぐと必ず真になって offline() に落ちない
